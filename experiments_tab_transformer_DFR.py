...
def parse_config():
    ...
    parser.add_argument("--output_path", default="output/DFR", type=str)
...
def pretrain_JTT(test_df, feature_names, error_samples, train_dataset, data_dir, device,
    ...
    for feature_index, feature_name in enumerate(feature_names):
        ...
        trained_model = do_MLM(
            ...
            save_dir,
            saved_model_path, categories=feature_names, max_epoch=args.max_epoch_phase1A, test_df=test_df,
...
def create_balanced_set(df, feature_name, output_col):
    groups = df.groupby([feature_name, output_col])
    min_count = min(groups.size())
    balanced_indices = []

    for _, group_indices in groups.groups.items():
        sampled_indices = random.sample(list(group_indices), min_count)
...
def print_time_taken(start_time, end_time, task_name):
    ...
    print(f"Time taken to load {task_name}: {int(hours)}h {int(minutes)}m {int(seconds)}s")
...
def perform_phase2(
    ...
    print(
        "\n====================== [INFO] Phase2A Balanced Set Creation for DFR on FTTransformer =================")
    ...
    if not all_models_exists:
        for feature_index, feature_name in enumerate(feature_names):
            print("===========" * 40)

            print(f"\n[INFO] Creating balanced set and training model for {feature_name}")

            # Create a balanced dataset from validation data
            balanced_val_indices = create_balanced_set(val_loader.dataset.data, feature_name, args.output_col)
            balanced_val_dataset = Subset(val_loader.dataset, balanced_val_indices)

            # Create DataLoader for the balanced dataset
            balanced_val_loader = DataLoader(balanced_val_dataset, batch_size=args.batch_size, shuffle=True)

            # Initialize and train model
            _, masked_feature_model, optimizer = get_model_optim(args, num_cat, feature_map_counts, device,
                                                                 feature_index=[feature_index])
            hyper_parameter = f"mr-{args.mask_val}-dfr-{feature_name}"
            saved_model_path = save_dir / f"best_model-phase2A-MLM-FTT-feature-{feature_name}.pth"
            masked_feature_model = do_MLM(
                masked_feature_model, balanced_val_loader, optimizer, device, val_loader, test_loader,
                save_dir,
                saved_model_path, categories=feature_names, max_epoch=args.max_epoch_phase1A, test_df=test,
                text=f"MLM Training for {feature_name}", weights=None)

            print(f"Model for {feature_name} saved at {saved_model_path}")
            print("===========" * 40)
    ...
    for feature in feature_names:
        print("============" * 40)
        print(f"\n[INFO] validation MLM model for {feature}")
        model = loaded_models[feature]
        _, _ = validate_MLM_epoch(model, test_loader, device, save_dir,
    ...
    end_time = time.time()
    print_time_taken(start_time, end_time, "train embeddings")
    ...
    end_time = time.time()
    print_time_taken(start_time, end_time, "val embeddings")
    ...
    end_time = time.time()
    print_time_taken(start_time, end_time, "test embeddings")
