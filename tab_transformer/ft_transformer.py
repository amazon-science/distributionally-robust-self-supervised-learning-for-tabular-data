...
class SupervisedModel(nn.Module):
    def __init__(self, input_dim, output_dim=1, hidden_layers=[100], activation_fn=nn.ReLU):
        super().__init__()
...
class MaskedFeatureModel(nn.Module):
    def __init__(self, base_model, feature_map_counts, n_cont_features, device, trainable_feature_indices=None):
        super().__init__()
        self.base_model = base_model
        self.feature_map_counts = feature_map_counts
        self.n_cont_features = n_cont_features
        self.device = device
        self.trainable_feature_indices = trainable_feature_indices

        total_cat_features = sum(feature_map_counts.values())

        self.reconstruction_layer_cont = nn.Linear(base_model.backbone.output[2].out_features, n_cont_features)
        self.reconstruction_layer_cat = nn.Linear(base_model.backbone.output[2].out_features, total_cat_features)

        if self.trainable_feature_indices is not None:
            print("[FTT Model] Setting gradients for trainable feature indices: ",
                         self.trainable_feature_indices)

            for param in self.reconstruction_layer_cat.parameters():
                param.requires_grad = False

            start_indices = [0]
            for feature in list(feature_map_counts.keys()):
                start_indices.append(start_indices[-1] + feature_map_counts[feature])

            print(start_indices)
            # Enable gradients only for specified feature indices
            for idx in self.trainable_feature_indices:
                feature_name = list(feature_map_counts.keys())[idx]
                start_idx = start_indices[idx]
                end_idx = start_idx + feature_map_counts[feature_name]
                print(feature_name, start_idx, end_idx)
                self.reconstruction_layer_cat.weight[:, start_idx:end_idx].requires_grad = True
                if self.reconstruction_layer_cat.bias is not None:
                    self.reconstruction_layer_cat.bias[start_idx:end_idx].requires_grad = True

        self.to(device)

    def forward(self, x_cont, x_cat, mask_cont=None, mask_cat=None):
        if mask_cont is None or mask_cat is None:
            # If no masks are provided (e.g., during testing), use data as-is
            x_cont_masked = x_cont
            x_cat_masked = x_cat
        else:
            # Apply masks if provided (during training or validation)
            x_cont_masked = x_cont * (1 - mask_cont)
            mask_cat = mask_cat.long()
            x_cat_masked = x_cat * (1 - mask_cat) + mask_cat * 0

        features = self.base_model(x_cat_masked, x_cont_masked)
        reconstructed_cont = self.reconstruction_layer_cont(features)
        reconstructed_cat_logits = self.reconstruction_layer_cat(features)

        reconstructed_cats = []
        start = 0
        for _, cardinality in self.feature_map_counts.items():
            end = start + cardinality
            reconstructed_cats.append(reconstructed_cat_logits[:, start:end])
            start = end

        return reconstructed_cont, reconstructed_cats

    def generate_masks(self, batch_size, mask_val, mode='train'):
        if mode == 'test':
            # Return no masks during testing
            return None, None
        # Generate masks normally during training and validation
        mask_cont = torch.bernoulli(torch.full((batch_size, self.n_cont_features), mask_val)).to(self.device)
        mask_cat = torch.bernoulli(torch.full((batch_size, len(self.feature_map_counts)), mask_val)).to(self.device)
        return mask_cont, mask_cat
...
class CategoricalEmbeddings(nn.Module):
    ...
    **Examples**
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # # print(xxxx)
        # print([self.embeddings[i](x[..., i]) for i in range(n_features)])
        # print("-----------")
        # print(xxx)
        x = torch.stack(
...
class FTTransformer(nn.Module):
    ...
    def forward(self, x_cat: Optional[Tensor], x_cont: Optional[Tensor]) -> Tensor:
        """Do the forward pass."""
        ...
        for argname, argvalue, module in [
            ...
            if module is None:
                if argvalue is not None:
                    raise ValueError(
                        FTTransformerPaper._FORWARD_BAD_ARGS_MESSAGE.format(
